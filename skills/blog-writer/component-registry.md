...
These rules apply to every component in the `components/mdx/` directory. Violating them causes runtime errors in Next.js App Router.

### "use client" Directive

Any component that uses browser-only APIs must have `"use client"` as its first line. This includes:
- **Event handlers** (`onClick`, `onMouseEnter`, `onMouseLeave`, etc.)
- **React hooks** (`useState`, `useEffect`, `useRef`, `useId`, etc.)
- **Browser APIs** (`window`, `document`, `localStorage`, etc.)

WHY: Next.js App Router treats all components as Server Components by default. Server Components render on the server and cannot include JavaScript that runs in the browser. If a component has `onMouseEnter` but no `"use client"`, Next.js throws: "Event handlers cannot be passed to Client Component props."

Current components that require `"use client"`: `AccordionGroup`, `Chart`, `Checklist`, `SVGDiagram`, `References`, `StatCard`, `ImageWithCaption`.

### Non-ASCII Heading Support

The blog supports Korean, Japanese, and other non-ASCII content. Heading IDs (for TOC anchor links) are generated by `slugify()` in `lib/mdx.ts`, which preserves Korean Hangul (`\uac00-\ud7a3`) and Jamo (`\u3131-\u318e`). If you add support for additional scripts (CJK, Cyrillic, etc.), extend the Unicode ranges in both `slugify()` and `extractHeadings()`.

### MDX Component Registry Typing

The `MDX_COMPONENTS` record uses `Record<string, React.ComponentType<any>>`. Do not change `any` to `Record<string, unknown>` — MDX passes props dynamically at runtime, and strict typing here causes TypeScript errors under `strict: true` because component prop interfaces don't overlap with `Record<string, unknown>`.
